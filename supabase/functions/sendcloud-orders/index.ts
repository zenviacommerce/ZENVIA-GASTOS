...
function nonActionable(v:unknown){const s=statusCode(v);return s.includes('cancel')||s==='fulfilled'||s==='shipped'||s.includes('shipped')||s==='delivered';}
...
async function fetchOrders(history:boolean){const min=history?yearStart():daysAgo(7);return fetchPaged(`/orders?page_size=200&sort=-order_created_at&order_created_at_min=${encodeURIComponent(min)}`,history?35:8);}
async function fetchShipments(history:boolean){const min=history?yearStart():daysAgo(7);return fetchPaged(`/shipments?page_size=200&announced_after=${encodeURIComponent(`${min}T00:00:00Z`)}`,history?35:8);}
function shipmentMeta(s:any){
  const parcel=Array.isArray(s?.parcels)?s.parcels[0]:null;const option=s?.ship_with?.properties?.shipping_option_code||null;const code=carrierCode(option,parcel?.tracking_url);
  return {sendcloud_parcel_id:parcel?.id==null?null:Number(parcel.id),sendcloud_shipment_id:s?.id==null?null:String(s.id),tracking_number:parcel?.tracking_number||null,tracking_url:parcel?.tracking_url||null,shipping_option_code:option,contract_id:s?.ship_with?.properties?.contract_id==null?null:Number(s.ship_with.properties.contract_id),carrier_code:code,carrier_name:friendlyCarrier(code),shipping_service_name:option,fulfilled_at:parcel?.announced_at||s?.updated_at||null};
}

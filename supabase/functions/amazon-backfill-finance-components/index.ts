...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    let processed=0;let componentCount=0;let hasMore=false;let pagesProcessed=0;
    for(let page=0;page<pages;page+=1){const loaded=await loadPage(admin,cursor,limit);if(!loaded.rows.length){hasMore=false;break;}componentCount+=await replacePage(admin,loaded.rows);processed+=loaded.rows.length;pagesProcessed+=1;cursor=String(loaded.rows[loaded.rows.length-1].id);hasMore=loaded.hasMore;if(!hasMore)break;}
    return response({ok:true,processed,components:componentCount,cursor,hasMore,limit,pagesProcessed});
  }catch(error){return response({error:error instanceof Error?error.message:'Error interno del backfill financiero.'},401);}
});

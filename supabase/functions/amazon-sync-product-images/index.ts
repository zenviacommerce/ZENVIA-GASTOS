  ...
  try{
    ...
    if(!products.length){
      ...
      for(const item of items||[]){
        const asin=String(item.asin||''),sellerSku=String(item.seller_sku||'');
        const key=asin+'|'+sellerSku;
        if(asin&&sellerSku&&!cachedAsins.has(asin)&&!seen.has(key)){seen.add(key);products.push({asin,sellerSku});}
      }
    }
    ...
    for(const product of products){
      try{
        ...
        rows.push({owner_id:account.owner_id,amazon_account_id:account.id,asin:product.asin,marketplace_id:marketplaceId,image_url:image?.url||null,image_width:null,image_height:null,fetched_at:now,updated_at:now});
    ...
    return response({ok:true,processed:rows.length,withImage:rows.filter(r=>r.image_url).length,remaining:null,failed:failed.length,failures:failed.slice(0,10)});

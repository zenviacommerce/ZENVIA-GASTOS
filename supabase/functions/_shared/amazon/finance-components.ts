...
type EconomicNode={
  node:Breakdown;
  path:string[];
  category:FinanceComponentCategory;
...
export type FinanceComponentInput={
  ...
  component_category:FinanceComponentCategory;
...
function refundedSalesNode(node:Breakdown,path:string[]):EconomicNode{
  ...
  return {
    ...
    path,
    category:'refund',
    componentType:'Refunded Sales',
    tax:tax?amount(tax):null,
    taxCurrency:tax?currency(tax):null,
  };
}
...
function economicNodes(nodes:Breakdown[],path:string[]=[]):EconomicNode[]{
  ...
  for(let index=0;index<nodes.length;index+=1){
    ...
    if(/refunded\s*sales/i.test(nodeType)){
      out.push(refundedSalesNode(node,nodePath));
      continue;
    }
    ...
    if(category && nested.length===0){
      out.push({node,path:nodePath,category,componentType:nodeType,tax:null,taxCurrency:null});
      continue;
    }
...
function rawBreakdowns(transaction:any):Breakdown[]{
  return Array.isArray(transaction?.breakdowns)?transaction.breakdowns:[];
}

function syntheticAdsNode(transaction:any):EconomicNode|null{
  const transactionType=String(transaction?.transactionType||'');
  if(!/productadspayment|advertis|sponsored/i.test(transactionType))return null;
  const total=transaction?.totalAmount||null;
  return {
    node:{breakdownType:transactionType,breakdownAmount:total},
    path:[`transaction:${transactionType}`],
    category:'ads_payment_excluded',
    componentType:transactionType,
    tax:null,
    taxCurrency:null,
  };
}
...
export async function normalizeFinanceComponents(transaction:any,persistedTransaction:any,_job:any):Promise<FinanceComponentInput[]>{
  ...
  if(!nodes.length){
    const synthetic=syntheticAdsNode(transaction);
    if(synthetic)nodes.push(synthetic);
  }

  const now=new Date().toISOString();
  const result:FinanceComponentInput[]=[];
  for(const entry of nodes){
    const componentKey=await sha256(JSON.stringify({
      transactionKey:persistedTransaction.transaction_key||persistedTransaction.id,
      path:entry.path,
      category:entry.category,
      componentType:entry.componentType,
    }));
    result.push({
      owner_id:String(persistedTransaction.owner_id),
      amazon_account_id:String(persistedTransaction.amazon_account_id),
      finance_transaction_id:String(persistedTransaction.id),
      marketplace_id:persistedTransaction.marketplace_id?String(persistedTransaction.marketplace_id):null,
      amazon_order_id:persistedTransaction.amazon_order_id?String(persistedTransaction.amazon_order_id):null,
      seller_sku:persistedTransaction.seller_sku?String(persistedTransaction.seller_sku):null,
      asin:persistedTransaction.asin?String(persistedTransaction.asin):null,
      posted_date:persistedTransaction.posted_date?String(persistedTransaction.posted_date):null,
      component_key:componentKey,
      component_type:entry.componentType,
      component_category:entry.category,
      amount_original:amount(entry.node),
      currency_code:currency(entry.node),
      tax_amount_original:entry.tax,
      amount_eur:null,
      tax_amount_eur:null,
      fx_rate:null,
      updated_at:now,

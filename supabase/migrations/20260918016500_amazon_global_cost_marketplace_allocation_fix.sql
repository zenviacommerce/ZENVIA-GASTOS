-- Do not replicate account-level/unassigned Amazon costs into every marketplace-filtered view.
-- They remain included in the all-marketplaces account total.

-- Attribute order-linked Amazon fees/refunds to the selected order cohort.
-- Keep only genuinely global/unlinked expenses on accounting/posting date.

-- Scope Amazon analytics completeness to sync jobs that overlap the selected period.
-- Inventory and unrelated historical backfills must not make current profitability provisional.
...
begin
  ...
  finance_rows as (
    -- Order-linked financial components follow the order cohort, not Amazon's posting date.
    ...
    union all
    -- Account/global costs remain attributed to their accounting date.
    select c.component_category,c.amount_original,c.tax_amount_original,
           private.amazon_rate_to_eur(c.currency_code,c.posted_date::date) as finance_fx
    from public.amazon_finance_components c
    where c.owner_id=v_owner
...
begin
  ...
  sync_bad as (
    ...
    where j.owner_id=v_owner
      ...
      and coalesce(j.window_to,'infinity'::timestamptz) >= from_date::timestamptz
      and coalesce(j.window_from,'-infinity'::timestamptz) < (to_date+1)::timestamptz
      and (marketplace_ids is null or cardinality(marketplace_ids)=0 or j.marketplace_id=any(marketplace_ids)) and j.status in ('queued','running','failed')) value
  )
  select jsonb_build_object(

...
import { countryName } from '../services/countryCatalog';
...
const SERIES_KIND_OPTIONS=[{value:'standard',label:'Ordinaria'},{value:'rectifying',label:'Rectificativa'}];
...
export function TaxRegistrationsPanel({onChanged}:{onChanged?:()=>void|Promise<void>}){
  ...
  const remove=async(item:TaxRegistration)=>{const confirmed=await confirmAction({title:'Eliminar registro IVA',message:`Se eliminará ${item.label} (${item.vatNumber}).`,confirmLabel:'Eliminar',tone:'danger'});if(!confirmed)return;setBusy(true);try{await deleteTaxRegistration(item.id);await refresh();await onChanged?.();showSuccess('Registro IVA eliminado.');if(editing===item.id)reset();}catch(e){showError(errorMessage(e,'No se pudo eliminar el registro IVA.'));}finally{setBusy(false);}};

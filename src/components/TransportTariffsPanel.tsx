...
import { activateTransportTariff, createTransportTariffDraft, deleteTransportTariffDraft, listTransportTariffs, markTransportTariffReviewed, parseTransportTariffDocument, saveTransportTariffReview, type TransportTariffBandDraft, type TransportTariffDocument, type TransportTariffFuelPeriod, type TransportTariffServiceDraft } from '../services/transportTariffs';
...
export function TransportTariffsPanel({open,onClose}:Props){
  ...
  const importFile=async(file:File)=>{
    ...
    }catch(e){const message=errorMessage(e,'No se pudo leer la tarifa.');setError(message);showError(message)}finally{setReading(false);if(fileRef.current)fileRef.current.value=''}
  };
  ...
  const updateFuelPeriod=(index:number,patch:Partial<TransportTariffFuelPeriod>)=>setDraft(current=>{if(!current)return current;const fuelPeriods=clone(current.fuelPeriods||[]);fuelPeriods[index]={...fuelPeriods[index],...patch};return {...current,fuelPeriods}});
  const addFuelPeriod=()=>setDraft(current=>{if(!current)return current;const existing=current.fuelPeriods||[];const last=existing.at(-1);let from=current.effectiveFrom||new Date().toISOString().slice(0,10);if(last){const base=last.effectiveTo||last.effectiveFrom;const d=new Date(`${base}T12:00:00`);d.setDate(d.getDate()+1);from=d.toISOString().slice(0,10)}return {...current,fuelPeriods:[...existing,{effectiveFrom:from,effectiveTo:null,fuelSurchargePct:current.fuelSurchargePct??0}]}});
  const removeFuelPeriod=(index:number)=>setDraft(current=>current?{...current,fuelPeriods:(current.fuelPeriods||[]).filter((_,i)=>i!==index)}:current);
  ...
  const save=async(showToast=true)=>{if(!draft)return;setSaving(true);setError('');try{await saveTransportTariffReview(draft);await refresh(draft.id);if(showToast)showSuccess('Revisión guardada.')}catch(e){const message=errorMessage(e,'No se pudo guardar la revisión.');setError(message);showError(message);throw e}finally{setSaving(false)}};
  ...
  const remove=async()=>{if(!selected||!confirm(`¿Eliminar la tarifa ${selected.carrierName} (${statusLabel(selected.status)})?`))return;setSaving(true);try{await deleteTransportTariffDraft(selected);await refresh(null);showSuccess('Tarifa eliminada.')}catch(e){showError(errorMessage(e,'No se pudo eliminar la tarifa.'))}finally{setSaving(false)}};
        ...
        <main className="transportTariffReview">
          ...
          {draft&&<>
            ...
            <section className="transportMetaGrid">
              ...
              <label>Combustible por defecto (%)<input disabled={!editable} inputMode="decimal" value={draft.fuelSurchargePct??''} placeholder="Configurable" onChange={e=>updateDoc({fuelSurchargePct:numberValue(e.target.value)})}/></label>
            ...
            <section className="transportFuelSection">
              <div className="transportServicesHead"><div><h3>Combustible por periodos</h3><p>Define el porcentaje que corresponde a cada periodo. Puedes corregir un mes pasado o añadir el siguiente sin crear otra tarifa.</p></div>{editable&&<button className="secondary" onClick={addFuelPeriod}><Plus size={15}/> Periodo</button>}</div>
              {(draft.fuelPeriods||[]).length>0?<div className="transportFuelPeriods"><div className="transportFuelHeader"><span>Desde</span><span>Hasta</span><span>Combustible %</span><span></span></div>{(draft.fuelPeriods||[]).map((period,index)=><div className="transportFuelRow" key={period.id||index}><input disabled={!editable} type="date" min={draft.effectiveFrom||undefined} max={draft.effectiveTo||undefined} value={period.effectiveFrom} onChange={e=>updateFuelPeriod(index,{effectiveFrom:e.target.value})}/><input disabled={!editable} type="date" min={period.effectiveFrom||draft.effectiveFrom||undefined} max={draft.effectiveTo||undefined} value={period.effectiveTo||''} onChange={e=>updateFuelPeriod(index,{effectiveTo:e.target.value||null})}/><input disabled={!editable} inputMode="decimal" value={period.fuelSurchargePct} onChange={e=>updateFuelPeriod(index,{fuelSurchargePct:Number(e.target.value.replace(',','.'))||0})}/>{editable?<button className="dangerIcon" onClick={()=>removeFuelPeriod(index)} aria-label="Eliminar periodo de combustible"><Trash2 size={15}/></button>:<span/>}</div>)}</div>:<div className="transportFuelEmpty">Sin periodos específicos. Se usa el porcentaje por defecto de la tarifa.</div>}
              {draft.status==='active'&&<small className="transportActiveEditHint">Al guardar una tarifa activa se recalculan los envíos cuyo coste sea una estimación de tarifa. Los costes reales del transportista no se modifican.</small>}
            </section>
            ...
            <footer className="transportTariffFooter">{['draft','reviewed'].includes(draft.status)&&<button className="danger" onClick={remove} disabled={saving}><Trash2 size={16}/> Eliminar</button>}<div className="transportFooterMain">{editable&&<button className="secondary" onClick={()=>void save()} disabled={saving}>{saving?<LoaderCircle className="spin" size={16}/>:null} Guardar cambios</button>}{draft.status==='draft'&&<button className="primary" onClick={review} disabled={saving}>Confirmar revisión</button>}{draft.status==='reviewed'&&<button className="primary" onClick={activate} disabled={saving}>Activar tarifa</button>}{draft.status==='active'&&<span className="transportActiveNote"><Check size={16}/> Tarifa activa · editable directamente</span>}</div></footer>
          </>}
        </main>
      </div>
    </div>

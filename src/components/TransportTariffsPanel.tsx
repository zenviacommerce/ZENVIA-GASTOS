...
import { AlertCircle, Check, FileSpreadsheet, LoaderCircle, Pencil, Plus, RefreshCw, Trash2, Upload, X } from 'lucide-react';
import { activateTransportTariff, createTransportTariffDraft, createTransportTariffVersion, deleteTransportTariffDraft, listTransportTariffs, markTransportTariffReviewed, parseTransportTariffDocument, saveTransportTariffReview, type TransportTariffBandDraft, type TransportTariffDocument, type TransportTariffServiceDraft } from '../services/transportTariffs';
...
const nextDate=(value?:string|null)=>{if(!value)return todayIso();const d=new Date(`${value}T12:00:00`);d.setDate(d.getDate()+1);return d.toISOString().slice(0,10)};
...
export function TransportTariffsPanel({open,onClose}:Props){
  ...
  useEffect(()=>{const doc=documents.find(item=>item.id===selectedId);if(!doc||doc.status!=='active'){setVersionFrom('');return}const min=nextDate(doc.effectiveFrom),today=todayIso();let next=today<min?min:today;if(doc.effectiveTo&&next>doc.effectiveTo)next=doc.effectiveTo;setVersionFrom(next)},[selectedId,documents]);
  ...
  const createVersion=async()=>{if(!draft||draft.status!=='active')return;if(!versionFrom){showError('Indica desde qué fecha se aplicarán los cambios.');return}setSaving(true);setError('');try{const id=await createTransportTariffVersion(draft.id,versionFrom);await refresh(id);showSuccess(`Nueva versión creada desde ${new Date(`${versionFrom}T12:00:00`).toLocaleDateString('es-ES')}. Edita los cambios y actívala cuando esté correcta.`)}catch(e){const message=errorMessage(e,'No se pudo crear la nueva vigencia.');setError(message);showError(message)}finally{setSaving(false)}};
    ...
    <div className="modal transportTariffModal">
      ...
      <div className="transportTariffBody">
        ...
        <main className="transportTariffReview">
          ...
          {draft&&<>
            <div className="transportReviewTitle"><div><h3>{editable?'Revisar importación':'Detalle de tarifa'}</h3><p>{draft.sourceFileName} · {sourceLabel(draft)} · confianza {Math.round(draft.parserConfidence*100)}%</p></div><span className={`transportStatus ${draft.status}`}>{statusLabel(draft.status)}</span></div>
            ...
            <footer className="transportTariffFooter">{['draft','reviewed'].includes(draft.status)&&<button className="danger" onClick={remove} disabled={saving}><Trash2 size={16}/> Eliminar</button>}<div className="transportFooterMain">{editable&&<button className="secondary" onClick={()=>void save()} disabled={saving}>{saving?<LoaderCircle className="spin" size={16}/>:null} Guardar cambios</button>}{editable&&<button className="primary" onClick={review} disabled={saving}>Confirmar revisión</button>}{draft.status==='reviewed'&&<button className="primary" onClick={activate} disabled={saving}>Activar tarifa</button>}{draft.status==='active'&&<div className="transportActiveVersioning"><span className="transportActiveNote"><Check size={16}/> Esta versión está activa.</span><label><span>Aplicar cambios desde</span><input type="date" min={nextDate(draft.effectiveFrom)} max={draft.effectiveTo||undefined} value={versionFrom} onChange={e=>setVersionFrom(e.target.value)}/></label><button className="secondary" onClick={createVersion} disabled={saving||!versionFrom}><Pencil size={15}/> Editar desde esta fecha</button><small>Se creará una nueva versión editable. La tarifa actual conservará el histórico hasta el día anterior.</small></div>}</div></footer>
          </>}
        </main>
      </div>
    </div>

...
export function TransportTariffsPanel({open,onClose}:Props){
  ...
  const save=async(showToast=true)=>{if(!draft)return;setSaving(true);setError('');try{if(draft.status==='active'){if(!activeApplyFrom){showError('Indica desde qué fecha se aplican los cambios.');return}await saveActiveTransportTariff(draft,activeApplyFrom);await refresh(draft.id);if(showToast)showSuccess(`Cambios aplicados desde ${new Date(`${activeApplyFrom}T12:00:00`).toLocaleDateString('es-ES')} y estimaciones recalculadas.`)}else{await saveTransportTariffReview(draft);await refresh(draft.id);if(showToast)showSuccess('Revisión guardada.')}}catch(e){const message=errorMessage(e,draft.status==='active'?'No se pudo actualizar la tarifa activa.':'No se pudo guardar la revisión.');setError(message);showError(message);throw e}finally{setSaving(false)}};

...
export function AmazonSummary({filters,onLoaded}:{filters:AmazonAnalyticsFilters;onLoaded?:(summary:AmazonSummaryData)=>void}){
  ...
  const cards=[
    ['Ventas sin IVA',money.format(summary.netSales)],['Pedidos',integer.format(summary.orders)],['Unidades vendidas',integer.format(summary.units)],
    ['Tarifas Amazon',money.format(summary.amazonFees)],['Reembolsos',money.format(summary.refunds)],['Coste producto',money.format(summary.productCost)],
    ['Beneficio antes de Ads',money.format(summary.profitBeforeAds??0)],['Margen',summary.marginPct==null?'—':`${summary.marginPct.toFixed(1)} %`],
  ];
  return <div className="amazonSummary">
    <AmazonCompleteness data={summary}/>
    <div className="amazonKpiGrid">{cards.map(([label,value])=><article className="card amazonKpiCard" key={label}><span>{label}</span><strong>{value}</strong>{(label==='Beneficio antes de Ads'||label==='Margen')&&!summary.profitComplete&&<small>Incompleto</small>}</article>)}</div>
    <section className="card amazonChartCard"><div className="amazonCardHeading"><div><span className="amazonSectionLabel">EVOLUCIÓN</span><strong>Ventas y beneficio</strong></div><span className="amazonCountBadge">{grain==='day'?'Diario':'Mensual'}</span></div>
      <div className="amazonChart"><ResponsiveContainer width="100%" height="100%"><LineChart data={series} margin={{top:8,right:16,bottom:0,left:0}}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="period" tickFormatter={value=>String(value).slice(5)}/><YAxis tickFormatter={value=>`${Math.round(Number(value))}€`}/><Tooltip formatter={(value:any)=>money.format(Number(value))}/><Legend/><Line type="monotone" dataKey="netSales" name="Ventas sin IVA" stroke="currentColor" strokeWidth={2} dot={false}/><Line type="monotone" dataKey="profitBeforeAds" name="Beneficio antes de Ads" stroke="currentColor" strokeDasharray="6 4" strokeWidth={2} dot={false}/></LineChart></ResponsiveContainer></div>
    </section>
    <div className="amazonBreakdownGrid"><section className="card amazonMiniCard"><span className="amazonSectionLabel">DESGLOSE DE COSTES</span><div className="amazonCostRows"><div><span>Tarifas Amazon</span><strong>{money.format(summary.amazonFees)}</strong></div><div><span>Coste producto</span><strong>{money.format(summary.productCost)}</strong></div><div><span>Reembolsos</span><strong>{money.format(summary.refunds)}</strong></div></div></section>
      <section className="card amazonMiniCard"><span className="amazonSectionLabel">MAYOR BENEFICIO</span>{topProfit.length?topProfit.map(row=><div className="amazonRankRow" key={`${row.sellerSku}-${row.asin}`}><span>{row.productName||row.sellerSku}</span><strong>{money.format(row.profitBeforeAds)}</strong></div>):<p className="amazonEmpty">Aún no hay productos con beneficio completo.</p>}</section>
      <section className="card amazonMiniCard"><span className="amazonSectionLabel">MENOR MARGEN</span>{weakMargin.length?weakMargin.map(row=><div className="amazonRankRow" key={`${row.sellerSku}-${row.asin}`}><span>{row.productName||row.sellerSku}</span><strong>{row.marginPct?.toFixed(1)} %</strong></div>):<p className="amazonEmpty">Aún no hay productos con margen completo.</p>}</section>
    </div>
  </div>;
}

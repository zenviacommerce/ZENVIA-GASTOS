...
export async function createSalesInvoiceDraft(input:SalesInvoiceDraftInput,defaultDueDays=30){const {data:invoice,error}=await supabase.from('sales_invoices').insert(invoiceRow(input,defaultDueDays)).select('id').single();if(error)throw error;const rows=lineRows(invoice.id,input.lines).filter(row=>row.description);if(rows.length){const {error:lineError}=await supabase.from('sales_invoice_lines').insert(rows);if(lineError){await supabase.from('sales_invoices').delete().eq('id',invoice.id);throw lineError;}}return invoice.id as string;}
export async function updateSalesInvoiceDraft(id:string,input:SalesInvoiceDraftInput,defaultDueDays=30){const {error}=await supabase.from('sales_invoices').update(invoiceRow(input,defaultDueDays)).eq('id',id).eq('status','draft');if(error)throw error;const {error:deleteError}=await supabase.from('sales_invoice_lines').delete().eq('invoice_id',id);if(deleteError)throw deleteError;const rows=lineRows(id,input.lines).filter(row=>row.description);if(rows.length){const {error:lineError}=await supabase.from('sales_invoice_lines').insert(rows);if(lineError)throw lineError;}}

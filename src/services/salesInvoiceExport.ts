...
export async function exportSalesInvoices(selected:SalesInvoice[],settings:BusinessSettings,branding:InvoicePdfBranding,label='seleccion'){
  const zip=new JSZip();
  ...
  zip.file('LEEME.txt',`ZENVIA Facturación · ${label}\nFacturas: ${selected.length}\nTotal: ${selected.reduce((sum,invoice)=>sum+invoice.totalAmount,0).toFixed(2)} EUR\n`);

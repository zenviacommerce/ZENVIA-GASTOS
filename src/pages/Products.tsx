...
export function Products({products,onAdd,onEdit,onDelete}:{products:Product[];onAdd:()=>void;onEdit:(product:Product)=>void;onDelete:(product:Product)=>Promise<void>}){
 ...
 {shown.length>0&&<div className="masterMobileList">{paged.map(p=>{const extra=salesMap.get(p.id);const metric=productMetrics(p,extra);return <button className="card masterMobileRow" key={p.id} onClick={()=>setSelected(p)}><div className="masterEntityCell"><div className="masterAvatar"><Package size={17}/></div><div><strong title={p.name}>{p.name}</strong><small>{p.sku||extra?.ean||'Sin SKU / EAN'} · {p.category||'Sin categoría'}</small></div></div><div className="masterMobileAmounts"><span>Coste <strong>{money(metric.cost,metric.cost!=null&&metric.cost<1?3:2)}</strong></span><span>P. venta <strong>{money(metric.sale)}</strong></span><span>Margen <strong>{metric.marginPct==null?'—':`${metric.marginPct.toFixed(1)} %`}</strong></span></div><ChevronRight size={18}/></button>})}</div>}

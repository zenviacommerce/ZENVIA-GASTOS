...
function isProcessedOrder(order:FulfillmentOrder){const status=orderStatusCode(order);return status==='fulfilled'||status==='shipped'||status==='delivered';}
...
function trackingState(order:FulfillmentOrder){
  ...
  if(raw.includes('ready to send')||raw.includes('announced')||raw.includes('being announced')||raw.includes('no label'))return {label:'Preparado',className:'ready'};
...
function OrderDrawer({order,onClose,onPrepare,onEdit,onPrint,onDownload,busy}:{order:FulfillmentOrder;onClose:()=>void;onPrepare:()=>void;onEdit:()=>void;onPrint:()=>void;onDownload:()=>void;busy:boolean}){
  ...
  return <div className="ordersDrawerBackdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><aside className="ordersDrawer">
    ...
    <div className="ordersDrawerKpis"><div><span>Estado</span><strong>{state.label}</strong></div><div><span>Total</span><strong>{money(order.totalAmount,order.currency||'EUR')}</strong></div><div><span>Peso</span><strong>{weightLabel(order)}</strong></div><div><span>Transportista</span><strong>{carrierLabel(order)}</strong></div></div>
...
export function Orders(){
  ...
  const filtered=useMemo(()=>{const q=query.trim().toLowerCase();return periodOrders.filter(order=>{if(channel!=='all'&&order.sourceChannel!==channel)return false;if(state==='pending'&&!isPendingOrder(order))return false;if(state==='labelled'&&!order.sendcloudParcelId)return false;if(state==='shipped'&&!isProcessedOrder(order))return false;if(state==='cancelled'&&!isCancelledOrder(order))return false;if(trackingFilter!=='all'&&trackingFilterCode(order)!==trackingFilter)return false;if(q&&!`${order.orderNumber||''} ${order.orderId||''} ${order.customerName||''} ${order.trackingNumber||''} ${order.trackingStatusMessage||''} ${carrierLabel(order)} ${productsText(order)}`.toLowerCase().includes(q))return false;return true})},[periodOrders,query,channel,state,trackingFilter]);
  ...
  return <div className="page ordersPage">
    ...
    <section className="card tableCard ordersTableCard">{loading?<div className="emptyState large"><LoaderCircle className="spin"/> Cargando pedidos…</div>:filtered.length?<table className="ordersTable"><thead><tr><th>Canal</th><th>Pedido</th><th>Cliente</th><th>Productos</th><th>Destino</th><th>Transportista</th><th>Peso</th><th className="right">Unidades</th><th className="right">Total</th><th>Estado</th><th>Seguimiento</th><th>Fecha</th><th></th></tr></thead><tbody>{filtered.map(order=>{const stateInfo=orderState(order),tracking=trackingState(order),first=order.items[0],rest=order.items.slice(1,3);return <tr key={order.id} className="clickableRow" onClick={()=>setSelected(order)}><td><span className={`ordersChannel ${order.sourceChannel}`}>{channelLabel(order)}</span></td><td><strong>{order.orderNumber||order.orderId||order.sendcloudId}</strong><small>{order.integrationName||''}</small></td><td>{order.customerName||text(order.shippingAddress.name)||'—'}</td><td className="ordersProductsCell" title={productsText(order)}>{first?<><strong>{itemLabel(first)}{itemQty(first)>1?` ×${itemQty(first)}`:''}</strong>{rest.length>0&&<small>{rest.map(item=>`${itemLabel(item)}${itemQty(item)>1?` ×${itemQty(item)}`:''}`).join(' · ')}{order.items.length>3?` · +${order.items.length-3} más`:''}</small>}</>:'—'}</td><td>{text(order.shippingAddress.country_code)||'—'} · {text(order.shippingAddress.postal_code)||''}</td><td><strong className="ordersCarrierText">{carrierLabel(order)}</strong></td><td><strong>{weightLabel(order)}</strong></td><td className="right"><strong>{order.items.reduce((sum,item)=>sum+itemQty(item),0)}</strong></td><td className="right"><strong>{money(order.totalAmount,order.currency||'EUR')}</strong></td><td><span className={`ordersState ${stateInfo.className}`}>{stateInfo.className==='ready'?<PackageCheck size={13}/>:stateInfo.className==='pending'?<Truck size={13}/>:<AlertCircle size={13}/>} {stateInfo.label}</span></td><td><span className={`ordersTracking ${tracking.className}`} title={order.trackingStatusMessage||tracking.label}>{tracking.label}</span></td><td>{dateLabel(order.orderCreatedAt)}</td><td className="right"><ChevronRight size={17}/></td></tr>})}</tbody></table>:<div className="emptyState large">No hay pedidos para estos filtros y fechas.</div>}</section>
    <div className="ordersMobileList">{filtered.map(order=>{const stateInfo=orderState(order),tracking=trackingState(order);return <button className="card ordersMobileRow" key={order.id} onClick={()=>setSelected(order)}><div><span className={`ordersChannel ${order.sourceChannel}`}>{channelLabel(order)}</span><strong>{order.orderNumber||order.orderId}</strong><small>{order.customerName||'Cliente'} · {weightLabel(order)} · {carrierLabel(order)}</small>{order.items[0]&&<small className="ordersMobileProduct">{itemLabel(order.items[0])}{order.items.length>1?` · +${order.items.length-1} producto${order.items.length-1===1?'':'s'}`:''}</small>}</div><div><b>{money(order.totalAmount,order.currency||'EUR')}</b><span className={`ordersState ${stateInfo.className}`}>{stateInfo.label}</span><span className={`ordersTracking ${tracking.className}`}>{tracking.label}</span></div><ChevronRight size={18}/></button>})}</div>
    ...
    {editOrder&&<OrderEditModal order={editOrder} saving={editSaving} onClose={()=>setEditOrder(null)} onSave={saveEdit}/>} 
  </div>;
}
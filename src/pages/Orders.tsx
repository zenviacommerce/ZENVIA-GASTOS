...
export function Orders(){
  ...
  const filtered=useMemo(()=>{const q=query.trim().toLowerCase();return periodOrders.filter(order=>{if(channel!=='all'&&order.sourceChannel!==channel)return false;if(state==='pending'&&!isPendingOrder(order))return false;if(state==='labelled'&&!order.sendcloudParcelId)return false;if(state==='shipped'&&!isProcessedOrder(order))return false;if(state==='cancelled'&&!isCancelledOrder(order))return false;if(q&&!`${order.orderNumber||''} ${order.orderId||''} ${order.customerName||''} ${order.trackingNumber||''} ${order.trackingStatusMessage||''} ${carrierLabel(order)} ${productsText(order)}`.toLowerCase().includes(q))return false;return true})},[periodOrders,query,channel,state]);

...
import {
  ...
  shouldRunHistorySync, syncSendcloudOrders, updateFulfillmentOrder, validateFulfillmentOrder,
  type FulfillmentOrder, type LocalPrinter, type ManualOrderItem, type OrderUpdateInput,
  type RemoteOrderValidation, type SendcloudStatus, type ShippingOption,
} from '../services/orders';
import { labelPdfBaseName, selectAutomaticShippingOption, uniqueLabelPdfFilename } from '../services/orderLabelFiles';
import {
  defaultCarrierForOrder, orderShippingCostDisplay, validateOrderForCarrier,
  type OrderShippingCostDisplay, type OrderValidationIssue,
...
function remoteIssues(validation?:RemoteOrderValidation):OrderValidationIssue[]{
  if(!validation||validation.unavailable)return [];
  const invalid=validation.inputAddressIsValid===false||validation.isValid===false;
  if(invalid)return [{code:'sendcloud_address_invalid',field:'address',label:'Dirección Sendcloud',message:validation.reasons[0]||'Sendcloud no ha podido validar esta dirección. Revísala antes de generar la etiqueta.',severity:'error'}];
  if(validation.changedAttributes.length||validation.reasons.length)return [{code:'sendcloud_address_warning',field:'address',label:'Dirección Sendcloud',message:validation.reasons[0]||`Sendcloud recomienda revisar: ${validation.changedAttributes.join(', ')}.`,severity:'warning'}];
  return [];
}

function ShippingCost({value}:{value:OrderShippingCostDisplay}){
  return <div className={`ordersShippingCost ${value.estimated?'estimated':''}`} title={value.detail}><strong>{money(value.gross,value.currency)}</strong><small>{value.estimated?'Estimado':'Real'} · {value.detail}</small></div>;
}

function OrderDrawer({order,shipping,issues,onClose,onPrepare,onEdit,onPrint,onDownload,busy}:{order:FulfillmentOrder;shipping:OrderShippingCostDisplay;issues:OrderValidationIssue[];onClose:()=>void;onPrepare:()=>void;onEdit:()=>void;onPrint:()=>void;onDownload:()=>void;busy:boolean}){
  const labelled=Boolean(order.sendcloudParcelId),state=orderState(order),tracking=trackingState(order),canPrepare=canPrepareOrder(order),blocking=issues.some(item=>item.severity==='error');
  return <div className="ordersDrawerBackdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><aside className="ordersDrawer">
    ...
    <div className="ordersDrawerKpis"><div><span>Estado</span><strong>{state.label}</strong></div><div><span>Total</span><strong>{money(order.totalAmount,order.currency||'EUR')}</strong></div><div><span>Peso</span><strong>{weightLabel(order)}</strong></div><div><span>Transportista</span><strong>{carrierLabel(order)}</strong></div><div><span>Envío</span><strong>{money(shipping.gross,shipping.currency)}</strong></div></div>
    {issues.length>0&&<section className="ordersDrawerSection"><div className="ordersSectionHead"><h3>Revisar antes de enviar</h3><span>{issues.length} aviso{issues.length===1?'':'s'}</span></div><div className="ordersValidationPanel">{issues.map(item=><div className={`ordersValidationIssue ${item.severity}`} key={item.code}><AlertCircle size={16}/><span><strong>{item.label}</strong><small>{item.message}</small></span></div>)}</div></section>}
    ...
    <section className="ordersDrawerSection"><div className="ordersSectionHead"><h3>Coste de envío</h3><span>{shipping.estimated?'Estimado':'Registrado'}</span></div><div className="ordersShippingBreakdown"><div><span>Total</span><strong>{money(shipping.gross,shipping.currency)}</strong></div><div><span>Base sin IVA</span><strong>{money(shipping.net,shipping.currency)}</strong></div><div><span>IVA</span><strong>{money(shipping.tax,shipping.currency)}</strong></div></div><small>{shipping.detail}</small></section>
...
export function Orders(){
  ...
  const [tariffs,setTariffs]=useState<TransportTariffDocument[]>([]);
  const [remoteValidations,setRemoteValidations]=useState<Record<string,RemoteOrderValidation>>({});
  const [previewOptions,setPreviewOptions]=useState<Record<string,ShippingOption|null>>({});
  const validationRequested=useRef(new Set<string>()),previewRequested=useRef(new Set<string>());
  ...
  const refreshTariffs=useCallback(async()=>{try{setTariffs(await listTransportTariffs())}catch{/* El precio real sigue disponible aunque no haya tarifa importada. */}},[]);
  ...
  const localIssues=(order:FulfillmentOrder)=>validateOrderForCarrier(order,defaultCarrierForOrder(order)).issues;
  const allIssues=(order:FulfillmentOrder)=>[...localIssues(order),...remoteIssues(remoteValidations[order.id])];
  const shippingFor=(order:FulfillmentOrder)=>orderShippingCostDisplay(order,tariffs,previewOptions[order.id]);
  const validationKey=(order:FulfillmentOrder)=>`${order.id}:${order.orderUpdatedAt||order.lastSyncedAt||''}`;

  const checkRemote=useCallback(async(order:FulfillmentOrder)=>{
    const carrier=defaultCarrierForOrder(order);
    try{const result=await validateFulfillmentOrder(order.id,carrier);setRemoteValidations(current=>({...current,[order.id]:result}));return result}
    catch{return null}
  },[]);

  useEffect(()=>{
    if(!status?.configured)return;
    for(const order of pendingOrders.slice(0,50)){
      const key=validationKey(order);if(validationRequested.current.has(key))continue;validationRequested.current.add(key);void checkRemote(order);
    }
  },[status?.configured,pendingOrders,checkRemote]);

  useEffect(()=>{
    if(!status?.configured)return;
    for(const order of pendingOrders.slice(0,30)){
      if(order.shippingCostAmount!=null||orderShippingCostDisplay(order,tariffs).gross!=null)continue;
      const key=validationKey(order);if(previewRequested.current.has(key))continue;previewRequested.current.add(key);
      void getShippingOptions(order.id).then(result=>setPreviewOptions(current=>({...current,[order.id]:selectAutomaticShippingOption(order,result.options)}))).catch(()=>setPreviewOptions(current=>({...current,[order.id]:null})));
    }
  },[status?.configured,pendingOrders,tariffs]);
  ...
  const prepare=async(order:FulfillmentOrder)=>{
    ...
    const local=localIssues(order);if(local.some(item=>item.severity==='error')){setEditOrder(order);showError(`Revisa el pedido antes de generar la etiqueta: ${local[0].message}`);return}
    const remote=remoteValidations[order.id]||await checkRemote(order);
    if(remote&&(remote.inputAddressIsValid===false||remote.isValid===false)){setEditOrder(order);showError(remote.reasons[0]||'Sendcloud no valida la dirección. Revísala antes de generar la etiqueta.');return}
    setLabelOrder(order);setOptions([]);setOptionsLoading(true);
    try{const result=await getShippingOptions(order.id);setOptions(result.options);const automatic=selectAutomaticShippingOption(order,result.options);if(automatic)setPreviewOptions(current=>({...current,[order.id]:automatic}))}
  ...
  const generatePendingLabels=async()=>{
    ...
    for(const order of pendingOrders){
      try{
        const local=localIssues(order);if(local.some(item=>item.severity==='error'))throw new Error(local[0].message);
        const remote=remoteValidations[order.id]||await checkRemote(order);if(remote&&(remote.inputAddressIsValid===false||remote.isValid===false))throw new Error(remote.reasons[0]||'Sendcloud no valida la dirección.');
  ...
  const saveEdit=async(value:OrderUpdateInput)=>{if(!editOrder)return;const editedId=editOrder.id;setEditSaving(true);try{await updateFulfillmentOrder(editedId,value);const freshOrders=await listFulfillmentOrders();setOrders(freshOrders);const fresh=freshOrders.find(item=>item.id===editedId)||editOrder;setSelected(fresh);setEditOrder(null);setRemoteValidations(current=>{const next={...current};delete next[editedId];return next});setPreviewOptions(current=>{const next={...current};delete next[editedId];return next});for(const key of Array.from(validationRequested.current))if(key.startsWith(`${editedId}:`))validationRequested.current.delete(key);for(const key of Array.from(previewRequested.current))if(key.startsWith(`${editedId}:`))previewRequested.current.delete(key);void checkRemote(fresh);showSuccess('Pedido actualizado en ZENVIA y Sendcloud.')}catch(e){showError(errorMessage(e,'No se pudo actualizar el pedido.'))}finally{setEditSaving(false)}};
  ...
  return <div className="page ordersPage">
    ...
    {printer&&<div className="ordersPrinterBar"><Printer size={15}/><span>Impresora directa:</span>{printers.length?<SearchableSelect value={printer} options={printers.map(item=>({value:item.id,label:`${item.name}${item.default?' · predeterminada':''}`,searchText:item.name}))} onChange={value=>{setPrinter(value);savePrinter(value)}} searchPlaceholder="Buscar impresora…" ariaLabel="Impresora directa"/>:<strong>{printer}</strong>}<button className="link" onClick={()=>{setPrinter('');savePrinter('')}}>Usar PDF</button></div>}

    <section className="card tableCard ordersTableCard">{loading?<div className="emptyState large"><LoaderCircle className="spin"/> Cargando pedidos…</div>:filtered.length?<table className="ordersTable"><thead><tr><th>Canal</th><th>Pedido</th><th>Cliente</th><th>Productos</th><th>Destino</th><th>Transportista</th><th>Envío</th><th>Peso</th><th className="right">Unidades</th><th className="right">Total</th><th>Estado</th><th>Seguimiento</th><th>Fecha pedido</th><th>Fecha etiqueta</th><th></th></tr></thead><tbody>{filtered.map(order=>{const stateInfo=orderState(order),tracking=trackingState(order),first=order.items[0],rest=order.items.slice(1,3),shipping=shippingFor(order),issues=allIssues(order),blocking=issues.some(item=>item.severity==='error');return <tr key={order.id} className="clickableRow" onClick={()=>setSelected(order)}><td><span className={`ordersChannel ${order.sourceChannel}`}>{channelLabel(order)}</span></td><td><strong>{order.orderNumber||order.orderId||order.sendcloudId}</strong><small>{order.integrationName||''}</small></td><td>{order.customerName||text(order.shippingAddress.name)||'—'}</td><td className="ordersProductsCell" aria-label={productsText(order)}>{first?<><strong>{itemLabel(first)}{itemQty(first)>1?` ×${itemQty(first)}`:''}</strong>{rest.length>0&&<small>{rest.map(item=>`${itemLabel(item)}${itemQty(item)>1?` ×${itemQty(item)}`:''}`).join(' · ')}{order.items.length>3?` · +${order.items.length-3} más`:''}</small>}</>:'—'}</td><td>{text(order.shippingAddress.country_code)||'—'} · {text(order.shippingAddress.postal_code)||''}</td><td><strong className="ordersCarrierText">{carrierLabel(order)}</strong></td><td><ShippingCost value={shipping}/></td><td><strong>{weightLabel(order)}</strong></td><td className="right"><strong>{order.items.reduce((sum,item)=>sum+itemQty(item),0)}</strong></td><td className="right"><strong>{money(order.totalAmount,order.currency||'EUR')}</strong></td><td><span className={`ordersState ${stateInfo.className}`}>{stateInfo.className==='ready'?<PackageCheck size={13}/>:stateInfo.className==='pending'?<Truck size={13}/>:<AlertCircle size={13}/>} {stateInfo.label}</span>{issues.length>0&&<span className={`ordersValidationBadge ${blocking?'error':''}`}><AlertCircle size={11}/> Revisar</span>}</td><td><span className={`ordersTracking ${tracking.className}`} aria-label={order.trackingStatusMessage||tracking.label}>{tracking.label}</span></td><td>{dateLabel(order.orderCreatedAt)}</td><td>{dateLabel(labelTimestamp(order))}</td><td className="right"><ChevronRight size={17}/></td></tr>})}</tbody></table>:<div className="emptyState large">No hay pedidos para estos filtros y fechas.</div>}</section>
    <div className="ordersMobileList">{filtered.map(order=>{const stateInfo=orderState(order),tracking=trackingState(order),shipping=shippingFor(order),issues=allIssues(order);return <button className="card ordersMobileRow" key={order.id} onClick={()=>setSelected(order)}><div><span className={`ordersChannel ${order.sourceChannel}`}>{channelLabel(order)}</span><strong>{order.orderNumber||order.orderId}</strong><small>{order.customerName||'Cliente'} · {weightLabel(order)} · {carrierLabel(order)}</small><small>Envío {money(shipping.gross,shipping.currency)} · Pedido {dateLabel(order.orderCreatedAt)} · Etiqueta {dateLabel(labelTimestamp(order))}</small>{order.items[0]&&<small className="ordersMobileProduct">{itemLabel(order.items[0])}{order.items.length>1?` · +${order.items.length-1} producto${order.items.length-1===1?'':'s'}`:''}</small>}{issues.length>0&&<span className={`ordersValidationBadge ${issues.some(item=>item.severity==='error')?'error':''}`}><AlertCircle size={11}/> Revisar</span>}</div><div><b>{money(order.totalAmount,order.currency||'EUR')}</b><span className={`ordersState ${stateInfo.className}`}>{stateInfo.label}</span><span className={`ordersTracking ${tracking.className}`}>{tracking.label}</span></div><ChevronRight size={18}/></button>})}</div>

    {selected&&<OrderDrawer order={orders.find(item=>item.id===selected.id)||selected} shipping={shippingFor(orders.find(item=>item.id===selected.id)||selected)} issues={allIssues(orders.find(item=>item.id===selected.id)||selected)} onClose={()=>setSelected(null)} onEdit={()=>setEditOrder(orders.find(item=>item.id===selected.id)||selected)} onPrepare={()=>prepare(orders.find(item=>item.id===selected.id)||selected)} onPrint={()=>existingLabel(orders.find(item=>item.id===selected.id)||selected,'print')} onDownload={()=>existingLabel(orders.find(item=>item.id===selected.id)||selected,'download')} busy={busyOrder===selected.id}/>} 
    ...
    {editOrder&&<OrderEditModal order={editOrder} issues={allIssues(editOrder)} saving={editSaving} onClose={()=>setEditOrder(null)} onSave={saveEdit}/>} 
  </div>;
}
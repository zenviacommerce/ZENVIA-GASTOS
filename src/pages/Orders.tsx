...
function OrderDrawer({order,onClose,onPrepare,onPrint,onDownload,busy}:{
  ...
  return <div className="ordersDrawerBackdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
    <aside className="ordersDrawer">
      ...
      <div className="ordersDrawerKpis"><div><span>Estado</span><strong>{labelled?'Etiqueta creada':order.sourceStatus||'Pendiente'}</strong></div><div><span>Total</span><strong>{money(order.totalAmount,order.currency||'EUR')}</strong></div><div><span>Productos</span><strong>{order.items.reduce((sum,item)=>sum+itemQty(item),0)}</strong></div></div>
...
export function Orders(){
  ...
  const [state,setState]=useState<'pending'|'labelled'|'all'>('pending');
  ...
  const filtered=useMemo(()=>{
    ...
    return orders.filter(order=>{
      ...
      if(state==='pending'&&order.sendcloudParcelId)return false;
  ...
  const pending=orders.filter(order=>!order.sendcloudParcelId).length;
  const amazon=orders.filter(order=>order.sourceChannel==='amazon'&&!order.sendcloudParcelId).length;
  const shopify=orders.filter(order=>order.sourceChannel==='shopify'&&!order.sendcloudParcelId).length;
  ...
  return <div className="page ordersPage">
    ...
    <section className="card tableCard ordersTableCard">{loading?<div className="emptyState large"><LoaderCircle className="spin"/> Cargando pedidos…</div>:filtered.length?<table className="ordersTable"><thead><tr><th>Canal</th><th>Pedido</th><th>Cliente</th><th>Destino</th><th className="right">Unidades</th><th className="right">Total</th><th>Estado</th><th>Fecha</th><th></th></tr></thead><tbody>{filtered.map(order=><tr key={order.id} className="clickableRow" onClick={()=>setSelected(order)}><td><span className={`ordersChannel ${order.sourceChannel}`}>{channelLabel(order.sourceChannel)}</span></td><td><strong>{order.orderNumber||order.orderId||order.sendcloudId}</strong><small>{order.integrationName||''}</small></td><td>{order.customerName||text(order.shippingAddress.name)||'—'}</td><td>{text(order.shippingAddress.country_code)||'—'} · {text(order.shippingAddress.postal_code)||''}</td><td className="right"><strong>{order.items.reduce((sum,item)=>sum+itemQty(item),0)}</strong></td><td className="right"><strong>{money(order.totalAmount,order.currency||'EUR')}</strong></td><td>{order.sendcloudParcelId?<span className="ordersState ready"><PackageCheck size={13}/> Etiquetado</span>:<span className="ordersState pending"><Truck size={13}/> Pendiente</span>}</td><td>{dateLabel(order.orderCreatedAt)}</td><td className="right"><ChevronRight size={17}/></td></tr>)}</tbody></table>:<div className="emptyState large">{status?.configured?'No hay pedidos para estos filtros.':'Cuando conectemos Sendcloud aparecerán aquí los pedidos de Amazon y Shopify.'}</div>}</section>

    <div className="ordersMobileList">{filtered.map(order=><button className="card ordersMobileRow" key={order.id} onClick={()=>setSelected(order)}><div><span className={`ordersChannel ${order.sourceChannel}`}>{channelLabel(order.sourceChannel)}</span><strong>{order.orderNumber||order.orderId}</strong><small>{order.customerName||text(order.shippingAddress.name)||'Cliente'} · {text(order.shippingAddress.country_code)}</small></div><div><b>{money(order.totalAmount,order.currency||'EUR')}</b><span className={`ordersState ${order.sendcloudParcelId?'ready':'pending'}`}>{order.sendcloudParcelId?'Etiquetado':'Pendiente'}</span></div><ChevronRight size={18}/></button>)}</div>

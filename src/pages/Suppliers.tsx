...
export function Suppliers({suppliers,onAdd,onEdit,onDelete}:{suppliers:Supplier[];onAdd:()=>void;onEdit:(supplier:Supplier)=>void;onDelete:(supplier:Supplier)=>Promise<void>}){
 ...
 return <div className="page">
   <div className="pageHead"><div><div className="eyebrow">MAESTRO</div><h1>Proveedores</h1><p>Datos y clasificación propios de cada proveedor.</p></div><button className="primary" onClick={onAdd}>+ Proveedor</button></div>
   ...
   {suppliers.length?<div className="supplierGrid">{suppliers.map(s=><div className="card supplierCard" key={s.id}>
     ...
     <div className="supplierInfo"><strong>{s.name}</strong><span>{supplierTypeLabel(s.supplierType)}{s.taxId?` · ${s.taxId}`:''}</span>{s.email&&<small>{s.email}</small>}{s.phone&&<small>{s.phone}</small>}</div>

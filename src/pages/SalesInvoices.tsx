...
import {
  Banknote, CheckCircle2, Download, Eye, FileCheck2, FilePenLine, PackageSearch,
  Pencil, Plus, ReceiptText, RotateCcw, Search, Send, Settings2, Trash2, X,
} from 'lucide-react';
import {
  addSalesPayment, createSalesInvoiceDraft, deleteSalesInvoiceDraft, downloadSalesInvoicePdf,
  ensureSalesSeries, issueSalesInvoice, loadBusinessSettings, loadClients, loadSalesInvoices,
  markSalesInvoiceSent, saveBusinessSettings, updateSalesInvoiceDraft,
...
function InvoiceModal({open,invoice,clients,products,onClose,onSaved}:{open:boolean;invoice:SalesInvoice|null;clients:Client[];products:BillableProduct[];onClose:()=>void;onSaved:()=>Promise<void>}){
 ...
 useEffect(()=>{
   ...
   }).catch(e=>!cancelled&&setError(e instanceof Error?e.message:'No se pudieron preparar las series.'));
 ...
 const chooseProduct=(index:number,productId:string)=>{
   if(!productId){updateLine(index,{productId:null});return;}
   const product=products.find(item=>item.id===productId);
   if(!product)return;
   updateLine(index,{productId:product.id,description:product.description,unit:product.unit,unitPrice:product.salePrice??0,taxRate:product.taxRate});
 };
 ...
 const save=async()=>{
   ...
   try{if(invoice)await updateSalesInvoiceDraft(invoice.id,payload);else await createSalesInvoiceDraft(payload);await onSaved();onClose();}
 ...
 return <div className="modalBackdrop"><div className="modal salesInvoiceModal">
   <div className="modalHead"><div><h3>{invoiceKind==='rectifying'?'Rectificativa en borrador':editing?'Editar borrador':'Nueva factura'}</h3><p>{invoiceKind==='rectifying'?'Revisa la corrección antes de emitirla. La serie R se asigna de forma independiente.':'El número definitivo se asignará únicamente cuando emitas la factura.'}</p></div><button onClick={onClose}><X/></button></div>
   ...
   <div className="salesLinesEditor">
    ...
    {lines.map((line,index)=>{const total=calcLine(line).total;return <div className="salesLine salesLineWithProduct" key={index}>
      <label className="salesLineProduct">Producto<select value={line.productId||''} onChange={e=>chooseProduct(index,e.target.value)}><option value="">Concepto libre</option>{products.map(product=><option key={product.id} value={product.id}>{product.name}{product.sku?` · ${product.sku}`:''}{product.salePrice==null?' · sin PVP':''}</option>)}</select></label>
   ...
   <div className="salesInvoiceBottom"><label>Notas<textarea rows={3} value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Observaciones visibles en la factura"/></label><div className="salesTotals"><span>Importe bruto <strong>{money(totals.gross)}</strong></span>{Math.abs(totals.gross-totals.net)>0.005&&<span>Descuento <strong>{money(totals.net-totals.gross)}</strong></span>}<span>Base imponible <strong>{money(totals.net)}</strong></span><span>IVA <strong>{money(totals.tax)}</strong></span><span className="salesGrandTotal">Total <strong>{money(totals.total)}</strong></span></div></div>
   {error&&<div className="errorBox">{error}</div>}
   <div className="modalActions"><button className="secondary" onClick={onClose} disabled={busy}>Cancelar</button><button className="primary" onClick={save} disabled={busy}>{busy?'Guardando…':'Guardar borrador'}</button></div>
 </div></div>;
}
...
function BusinessModal({open,settings,onClose,onSaved}:{open:boolean;settings:BusinessSettings;onClose:()=>void;onSaved:(settings:BusinessSettings)=>Promise<void>}){
 ...
 const save=async()=>{
  ...
  setBusy(true);try{await saveBusinessSettings(form);await onSaved(form);onClose();}catch(e){setError(e instanceof Error?e.message:'No se pudieron guardar los datos fiscales.');}finally{setBusy(false);}
 };
...
function PaymentModal({invoice,onClose,onSaved}:{invoice:SalesInvoice|null;onClose:()=>void;onSaved:()=>Promise<void>}){
 ...
 const save=async()=>{if(amount<=0){setError('Indica un importe superior a 0.');return;}if(amount>pending+0.005){setError('El cobro no puede superar el importe pendiente.');return;}setBusy(true);try{await addSalesPayment(invoice.id,{amount,paymentDate:date,method,reference});await onSaved();onClose();}catch(e){setError(e instanceof Error?e.message:'No se pudo registrar el cobro.');}finally{setBusy(false);}};
 return <div className="modalBackdrop"><div className="modal smallModal"><div className="modalHead"><div><h3>Registrar cobro</h3><p>{invoice.invoiceNumber} · Pendiente {money(pending)}</p></div><button onClick={onClose}><X/></button></div><div className="stackForm"><label>Importe<input type="number" min="0.01" max={pending} step="0.01" value={amount} onChange={e=>setAmount(Number(e.target.value))}/></label><label>Fecha<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><label>Método<input value={method} onChange={e=>setMethod(e.target.value)}/></label><label>Referencia<input value={reference} onChange={e=>setReference(e.target.value)} placeholder="Transferencia, operación…"/></label></div>{error&&<div className="errorBox">{error}</div>}<div className="modalActions"><button className="secondary" onClick={onClose}>Cancelar</button><button className="primary" disabled={busy} onClick={save}>{busy?'Guardando…':'Registrar cobro'}</button></div></div></div>;
}

function InvoiceDetail({invoice,settings,onClose,onPdf,onPayment,onMarkSent,onRectify,onEdit}:{invoice:SalesInvoice|null;settings:BusinessSettings;onClose:()=>void;onPdf:(invoice:SalesInvoice)=>void;onPayment:(invoice:SalesInvoice)=>void;onMarkSent:(invoice:SalesInvoice)=>void;onRectify:(invoice:SalesInvoice)=>void;onEdit:(invoice:SalesInvoice)=>void}){
 ...
 return <div className="modalBackdrop"><div className="modal salesDetailModal">
  <div className="modalHead"><div><div className="eyebrow">{invoice.invoiceType==='rectifying'?'FACTURA RECTIFICATIVA':'FACTURA DE VENTA'}</div><h3>{invoice.invoiceNumber||'Borrador'}</h3><p>{invoice.clientName} · {dateLabel(invoice.issueDate)}</p></div><button onClick={onClose}><X/></button></div>
  ...
  <div className="modalActions salesDetailActions">
   {invoice.status==='draft'?<button className="secondary" onClick={()=>onEdit(invoice)}><Pencil size={16}/> Editar borrador</button>:<button className="secondary" onClick={()=>onPdf(invoice)}><Download size={16}/> Descargar PDF</button>}
   {invoice.status==='issued'&&<button className="secondary" onClick={()=>onMarkSent(invoice)}><Send size={16}/> Marcar enviada</button>}
...
export function SalesInvoices(){
 ...
 const [modal,setModal]=useState(false);const [editing,setEditing]=useState<SalesInvoice|null>(null);const [detail,setDetail]=useState<SalesInvoice|null>(null);const [businessModal,setBusinessModal]=useState(false);const [paymentInvoice,setPaymentInvoice]=useState<SalesInvoice|null>(null);const [busyId,setBusyId]=useState<string|null>(null);
 const refresh=async()=>{
  setLoading(true);
  try{
  ...
  }catch(e){setError(e instanceof Error?e.message:'No se pudo cargar la facturación.');}
 ...
 const emit=async(invoice:SalesInvoice)=>{if(!window.confirm(`¿Emitir ${invoice.invoiceType==='rectifying'?'esta rectificativa':'esta factura'}? Se asignará el número definitivo y los datos fiscales quedarán bloqueados.`))return;setBusyId(invoice.id);setError('');try{await issueSalesInvoice(invoice.id);await refresh();}catch(e){setError(e instanceof Error?e.message:'No se pudo emitir la factura.');}finally{setBusyId(null);}};
 const remove=async(invoice:SalesInvoice)=>{if(!window.confirm('¿Eliminar este borrador?'))return;setBusyId(invoice.id);try{await deleteSalesInvoiceDraft(invoice.id);if(detail?.id===invoice.id)setDetail(null);await refresh();}catch(e){setError(e instanceof Error?e.message:'No se pudo eliminar el borrador.');}finally{setBusyId(null);}};
 ...
 return <div className="page">
  ...
  <section className="card tableCard">{loading?<div className="emptyState large">Cargando facturación…</div>:filtered.length?<table><thead><tr><th>Fecha</th><th>Número</th><th>Cliente</th><th>Tipo</th><th>Estado</th><th className="right">Base</th><th className="right">IVA</th><th className="right">Total</th><th className="right">Pendiente</th><th className="right">Acciones</th></tr></thead><tbody>{filtered.map(invoice=>{const pending=Math.max(0,invoice.totalAmount-invoice.paidAmount);return <tr key={invoice.id} className="clickableRow" onClick={()=>setDetail(invoice)}><td>{dateLabel(invoice.issueDate)}</td><td><strong>{invoice.invoiceNumber||'Borrador'}</strong></td><td>{invoice.clientName}</td><td>{invoice.invoiceType==='rectifying'?<span className="tag">Rectificativa</span>:'Ordinaria'}</td><td><span className={statusClass(invoice.status)}>{statusLabel(invoice.status)}</span></td><td className="right">{money(invoice.subtotal)}</td><td className="right">{money(invoice.taxAmount)}</td><td className="right"><strong>{money(invoice.totalAmount)}</strong></td><td className="right">{invoice.status==='draft'||invoice.invoiceType==='rectifying'?'—':money(pending)}</td><td className="right"><div className="invoiceActions" onClick={e=>e.stopPropagation()}>
    ...
    {invoice.status==='draft'?<><button className="iconBtn" title="Editar borrador" onClick={()=>edit(invoice)}><Pencil size={16}/></button><button className="iconBtn accountBtn" title="Emitir" disabled={busyId===invoice.id} onClick={()=>emit(invoice)}><FileCheck2 size={16}/></button><button className="iconBtn dangerIcon" title="Eliminar borrador" disabled={busyId===invoice.id} onClick={()=>remove(invoice)}><Trash2 size={16}/></button></>:<><button className="iconBtn" title="Descargar PDF" onClick={()=>pdf(invoice)}><Download size={16}/></button>{invoice.status==='issued'&&<button className="iconBtn" title="Marcar como enviada" disabled={busyId===invoice.id} onClick={()=>markSent(invoice)}><Send size={16}/></button>}{invoice.invoiceType==='standard'&&!['paid','rectified'].includes(invoice.status)&&<button className="iconBtn accountBtn" title="Registrar cobro" onClick={()=>setPaymentInvoice(invoice)}><CheckCircle2 size={16}/></button>}{invoice.invoiceType==='standard'&&!['rectified'].includes(invoice.status)&&<button className="iconBtn" title="Crear rectificativa" disabled={busyId===invoice.id} onClick={()=>rectify(invoice)}><RotateCcw size={16}/></button>}</>}
   </div></td></tr>;})}</tbody></table>:<div className="emptyState large">No hay facturas de venta para los filtros seleccionados.</div>}</section>
  {!products.length&&clients.length>0&&<div className="card alertCard"><div className="trendIcon"><PackageSearch/></div><div><h3>Catálogo comercial</h3><p>Puedes crear facturas con conceptos libres. Si añades precio de venta a tus productos, podrás seleccionarlos y cargar automáticamente descripción, precio e IVA.</p></div></div>}
  ...
  <InvoiceDetail invoice={detail} settings={settings} onClose={()=>setDetail(null)} onPdf={pdf} onPayment={setPaymentInvoice} onMarkSent={markSent} onRectify={rectify} onEdit={edit}/>

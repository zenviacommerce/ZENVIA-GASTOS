...
import {
  ...
  Mail, PackageSearch, Pencil, Plus, ReceiptText, RotateCcw, Search, Settings2,
...
function InvoiceDetail({invoice,settings,onClose,onPdf,onPayment,onSend,onRectify,onEdit,onDelete}:{invoice:SalesInvoice|null;settings:BusinessSettings;onClose:()=>void;onPdf:(invoice:SalesInvoice)=>void;onPayment:(invoice:SalesInvoice)=>void;onSend:(invoice:SalesInvoice)=>void;onRectify:(invoice:SalesInvoice)=>void;onEdit:(invoice:SalesInvoice)=>void;onDelete:(invoice:SalesInvoice)=>void}){
  ...
  return <div className="modalBackdrop"><div className="modal salesDetailModal polishedModal">
    ...
    <div className="modalActions salesDetailActions">
      {invoice.status==='draft'?<><button className="secondary" onClick={()=>onEdit(invoice)}><Pencil size={16}/> Editar borrador</button><button className="secondary dangerText" onClick={()=>onDelete(invoice)}><Trash2 size={16}/> Eliminar borrador</button></>:<><button className="secondary" onClick={()=>onPdf(invoice)}><Download size={16}/> Descargar PDF</button>{invoice.status!=='rectified'&&<button className="primary" onClick={()=>onSend(invoice)}><Mail size={16}/> Enviar por Gmail</button>}</>}
...
export function SalesInvoices(){
  ...
  const remove=async(invoice:SalesInvoice)=>{if(!window.confirm('¿Eliminar este borrador? Esta acción no se puede deshacer.'))return;setBusyId(invoice.id);setError('');try{await deleteSalesInvoiceDraftSafe(invoice.id);if(detail?.id===invoice.id)setDetail(null);await refresh();showSuccess('Borrador eliminado correctamente.');}catch(e){const message=errorMessage(e,'No se pudo eliminar el borrador.');setError(message);showError(message);}finally{setBusyId(null);}};
  ...
  return <div className="page">
    ...
    <InvoiceDetail invoice={detail} settings={settings} onClose={()=>setDetail(null)} onPdf={pdf} onPayment={setPaymentInvoice} onSend={setSendInvoice} onRectify={rectify} onEdit={edit} onDelete={remove}/>

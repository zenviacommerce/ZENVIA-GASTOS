...
import {
  Banknote, CheckCircle2, Download, Eye, FileCheck2, FilePenLine, Mail,
  PackageSearch, Pencil, Plus, ReceiptText, RotateCcw, Search, Settings2, Trash2, X,
} from 'lucide-react';
import {
...
function InvoiceDetail({invoice,settings,onClose,onPdf,onPayment,onSend,onRectify,onEdit}:{invoice:SalesInvoice|null;settings:BusinessSettings;onClose:()=>void;onPdf:(invoice:SalesInvoice)=>void;onPayment:(invoice:SalesInvoice)=>void;onSend:(invoice:SalesInvoice)=>void;onRectify:(invoice:SalesInvoice)=>void;onEdit:(invoice:SalesInvoice)=>void}){
 ...
 return <div className="modalBackdrop"><div className="modal salesDetailModal">
  ...
  <div className="modalActions salesDetailActions">
   {invoice.status==='draft'?<button className="secondary" onClick={()=>onEdit(invoice)}><Pencil size={16}/> Editar borrador</button>:<><button className="secondary" onClick={()=>onPdf(invoice)}><Download size={16}/> Descargar PDF</button>{invoice.status!=='rectified'&&<button className="primary" onClick={()=>onSend(invoice)}><Mail size={16}/> Enviar por Gmail</button>}</>}
...
export function SalesInvoices(){
 ...
 return <div className="page">
  ...
  <section className="card tableCard">{loading?<div className="emptyState large">Cargando facturación…</div>:filtered.length?<table><thead><tr><th>Fecha</th><th>Número</th><th>Cliente</th><th>Tipo</th><th>Estado</th><th className="right">Base</th><th className="right">IVA</th><th className="right">Total</th><th className="right">Pendiente</th><th className="right">Acciones</th></tr></thead><tbody>{filtered.map(invoice=>{const pending=Math.max(0,invoice.totalAmount-invoice.paidAmount);return <tr key={invoice.id} className="clickableRow" onClick={()=>setDetail(invoice)}><td>{dateLabel(invoice.issueDate)}</td><td><strong>{invoice.invoiceNumber||'Borrador'}</strong></td><td>{invoice.clientName}</td><td>{invoice.invoiceType==='rectifying'?<span className="tag">Rectificativa</span>:'Ordinaria'}</td><td><span className={statusClass(invoice.status)}>{statusLabel(invoice.status)}</span></td><td className="right">{money(invoice.subtotal)}</td><td className="right">{money(invoice.taxAmount)}</td><td className="right"><strong>{money(invoice.totalAmount)}</strong></td><td className="right">{invoice.status==='draft'||invoice.invoiceType==='rectifying'?'—':money(pending)}</td><td className="right"><div className="invoiceActions" onClick={e=>e.stopPropagation()}>
    ...
    {invoice.status==='draft'?<><button className="iconBtn" title="Editar borrador" onClick={()=>edit(invoice)}><Pencil size={16}/></button><button className="iconBtn accountBtn" title="Emitir" disabled={busyId===invoice.id} onClick={()=>emit(invoice)}><FileCheck2 size={16}/></button><button className="iconBtn dangerIcon" title="Eliminar borrador" disabled={busyId===invoice.id} onClick={()=>remove(invoice)}><Trash2 size={16}/></button></>:<><button className="iconBtn" title="Descargar PDF" onClick={()=>pdf(invoice)}><Download size={16}/></button>{invoice.status!=='rectified'&&<button className="iconBtn" title="Enviar por Gmail" onClick={()=>setSendInvoice(invoice)}><Mail size={16}/></button>}{invoice.invoiceType==='standard'&&!['paid','rectified'].includes(invoice.status)&&<button className="iconBtn accountBtn" title="Registrar cobro" onClick={()=>setPaymentInvoice(invoice)}><CheckCircle2 size={16}/></button>}{invoice.invoiceType==='standard'&&invoice.status!=='rectified'&&<button className="iconBtn" title="Crear rectificativa" disabled={busyId===invoice.id} onClick={()=>rectify(invoice)}><RotateCcw size={16}/></button>}</>}
  ...
  <InvoiceDetail invoice={detail} settings={settings} onClose={()=>setDetail(null)} onPdf={pdf} onPayment={setPaymentInvoice} onSend={setSendInvoice} onRectify={rectify} onEdit={edit}/>
  ...
  <SendInvoiceModal invoice={sendInvoice} settings={settings} onClose={()=>setSendInvoice(null)} onSent={refresh}/>
 </div>;
}

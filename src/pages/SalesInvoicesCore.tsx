...
function PaymentModal({invoice,onClose,onSaved}:{invoice:SalesInvoice|null;onClose:()=>void;onSaved:()=>Promise<void>}){
  ...
  useEffect(()=>{if(invoice){setAmount(Math.max(0,invoice.totalAmount-invoice.paidAmount));setDate(today());setMethod(invoice.paymentMethod||'Transferencia bancaria');setReference('');setError('');}},[invoice]); if(!invoice)return null;
  const save=async()=>{if(amount<=0){setError('Indica un importe superior a 0.');return;}if(amount>pending+0.005){setError('El cobro no puede superar el importe pendiente.');return;}setBusy(true);setError('');try{await addSalesPayment(invoice.id,{amount,paymentDate:date,method,reference});await onSaved();showSuccess(amount>=pending-0.005?'Factura marcada como cobrada.':'Cobro parcial registrado correctamente.');onClose();}catch(e){setError(errorMessage(e,'No se pudo registrar el cobro.'));}finally{setBusy(false);}};
  return <div className="modalBackdrop"><div className="modal smallModal polishedModal salesPaymentModal"><div className="modalHead salesModalHead"><div><div className="eyebrow">COBROS</div><h3>Registrar cobro</h3><p>{invoice.invoiceNumber} · Pendiente {money(pending)}</p></div><button onClick={onClose}><X/></button></div><div className="salesPaymentQuick"><button type="button" className="primary" onClick={()=>setAmount(pending)}>Cobrar todo · {money(pending)}</button><span>Puedes modificar el importe si el cliente ha hecho un pago parcial.</span></div><div className="salesFormSection"><div className="salesFormGrid"><label>Importe<input type="number" min="0.01" max={pending} step="0.01" value={amount} onChange={e=>setAmount(Number(e.target.value))}/></label><label>Fecha<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><label>Método<SelectField value={method} onChange={setMethod} ariaLabel="Método de cobro" options={[{value:'Transferencia bancaria',label:'Transferencia bancaria'},{value:'Tarjeta',label:'Tarjeta'},{value:'Efectivo',label:'Efectivo'},{value:'PayPal',label:'PayPal'},{value:'Bizum',label:'Bizum'},{value:'Domiciliación bancaria',label:'Domiciliación bancaria'},{value:'Otro',label:'Otro'}]}/></label><label>Referencia<input value={reference} onChange={e=>setReference(e.target.value)} placeholder="Nº operación, transferencia…"/></label></div></div>{error&&<div className="errorBox">{error}</div>}<div className="modalActions"><button className="secondary" onClick={onClose}>Cancelar</button><button className="primary" disabled={busy} onClick={save}>{busy?'Guardando…':amount>=pending-0.005?'Marcar como cobrada':'Registrar cobro parcial'}</button></div></div></div>;
}

function BulkPaymentModal({invoices,onClose,onSaved}:{invoices:SalesInvoice[];onClose:()=>void;onSaved:()=>Promise<void>}){
...
function InvoiceDetail({invoice,settings,onClose,onPdf,onPrint,onPayment,onSend,onRectify,onEdit,onReopen,onDelete,onIssue}:{invoice:SalesInvoice|null;settings:BusinessSettings;onClose:()=>void;onPdf:(invoice:SalesInvoice)=>void;onPrint:(invoice:SalesInvoice)=>void;onPayment:(invoice:SalesInvoice)=>void;onSend:(invoice:SalesInvoice)=>void;onRectify:(invoice:SalesInvoice)=>void;onEdit:(invoice:SalesInvoice)=>void;onReopen:(invoice:SalesInvoice)=>void;onDelete:(invoice:SalesInvoice)=>void;onIssue:(invoice:SalesInvoice)=>void}){
    ...
    {!loading&&filtered.length>0&&<div className="salesInvoicesMobileList">
      {filtered.map(invoice=>{const pending=Math.max(0,invoice.totalAmount-invoice.paidAmount);const collectable=invoice.invoiceType==='standard'&&!['draft','paid','rectified'].includes(invoice.status)&&pending>0.005;return <article className={`salesInvoiceMobileCard ${selectedSet.has(invoice.id)?'selected':''}`} key={invoice.id}>
        ...
        <button className="salesInvoiceMobileMain" type="button" onClick={()=>setDetail(invoice)}>
          ...
          <div className="salesInvoiceMobileMeta"><span>{dateLabel(invoice.issueDate)}</span><span>{invoice.invoiceType==='rectifying'?'Rectificativa':'Ordinaria'}</span></div>

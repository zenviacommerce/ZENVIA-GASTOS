...
export function SalesInvoices({
  ...
  const [modal,setModal]=useState(false);const [editing,setEditing]=useState<SalesInvoice|null>(null);const [detail,setDetail]=useState<SalesInvoice|null>(null);const [businessModal,setBusinessModal]=useState(false);const [seriesModal,setSeriesModal]=useState(false);const [paymentInvoice,setPaymentInvoice]=useState<SalesInvoice|null>(null);const [sendInvoice,setSendInvoice]=useState<SalesInvoice|null>(null);const [busyId,setBusyId]=useState<string|null>(null);const [bulkDeleting,setBulkDeleting]=useState(false);
  ...
  return <div className="page"><div className="pageHead"><div><div className="eyebrow">VENTAS</div><h1>Facturación</h1><p>Borradores, series, registros IVA, emisión, envío por Gmail, cobros y rectificativas desde un único sitio.</p></div><div className="actions"><button className="secondary" onClick={()=>setSeriesModal(true)}><ListOrdered size={17}/> Series</button><button className="secondary" onClick={()=>setBusinessModal(true)}><Settings2 size={17}/> Datos fiscales</button><button className="primary" onClick={openNew}>+ Nueva factura</button></div></div>
    ...
    {filtered.length>0&&<BulkSelectionToolbar selectedCount={selectedVisible.length} totalCount={filtered.length} allSelected={allVisibleSelected} onToggleAll={toggleAllVisible} label="facturas visibles">
      <button className="secondary dangerText" type="button" disabled={!deletableSelected.length||bulkDeleting} onClick={()=>void removeSelected()}><Trash2 size={15}/> {bulkDeleting?'Eliminando…':`Eliminar seleccionadas (${deletableSelected.length})`}</button>
      <button className="primary" type="button" disabled={!selectedVisible.length||bulkDeleting} onClick={()=>onExportSelected?.(selectedVisible.map(invoice=>invoice.id))}><Download size={15}/> Exportar seleccionadas ({selectedVisible.length})</button>

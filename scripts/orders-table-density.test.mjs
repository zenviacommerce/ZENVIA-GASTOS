...
test('tracking and date columns never overlap on desktop',async()=>{
  ...
  assert.match(css,/th:nth-child\(13\)[^}]*min-width:\s*230px/s);
  assert.match(css,/th:nth-child\(14\)[^}]*min-width:\s*145px/s);
  assert.match(css,/td:nth-child\(13\) \.ordersTracking\{[^}]*max-width:\s*210px[^}]*overflow:hidden/s);
});
